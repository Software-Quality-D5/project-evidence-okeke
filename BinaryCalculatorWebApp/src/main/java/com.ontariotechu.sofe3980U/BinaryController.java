	...
	@RequestParam(name="operand2", required=false, defaultValue="") String operand2, Model model) {
		...
		switch(operator)
		{
			case "+":
				...
				return "result";
			case "*":
				model.addAttribute("result", Binary.multiply(number1.getValue(),number2.getValue()).getValue());
				return "result";
			case "|":
				model.addAttribute("result", Binary.or(number1.getValue(),number2.getValue()).getValue());
				return "result";
			case "&":
				model.addAttribute("result", Binary.and(number1.getValue(),number2.getValue()).getValue());
				return "result";	
